...
KBUILD_CFLAGS   += -O3 $(call cc-disable-warning,maybe-uninitialized,) $(OPTS)
...
KBUILD_CFLAGS += $(call cc-disable-warning, count-overflow)
...
lto-clang-flags	:= -flto -flto=jobserver -fno-fat-lto-objects \
		   -fuse-linker-plugin -fwhole-program -fprofile-use=$(CLANG_PROFDATA) \
...
KBUILD_LDFLAGS += $(LD_FLAGS_LTO_CLANG) --allow-shlib-undefined 
...
ifdef CPU_NOT_KYRO
KBUILD_CFLAGS   += -mcpu=cortex-a55 -mcpu=cortex-a77
KBUILD_AFLAGS   += -mcpu=cortex-a55 -mcpu=cortex-a77
KBUILD_LDFLAGS  += -mllvm -mcpu=cortex-a55 -mcpu=cortex-a77
endif
